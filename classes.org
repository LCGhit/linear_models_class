...
* 11_11_2024
** ficha [[file:~/Documents/CD/year_2/semester_01/modelos_lineares_aplicacoes/exercicios/ExercicioAula.pdf][ExercícioAula]]
*** a)
...
**** ANOVA table
#+begin_src R :session 11_11_2024 :results output
  1-((650/(50-4-1))/((204.12+53.34+12.4+63.05+650)/(50-1)))
  anova(model)
  1-pf(abs(5.756), 4, 45)
#+end_src
...
* 04_12_2024
** [[file:~/Documents/CD/year_2/semester_01/modelos_lineares_aplicacoes/slides/variaveisqualitativasmultipla.pdf][exercise]] page 8
** 1
...
* resolução [[file:~/Documents/CD/year_2/semester_01/modelos_lineares_aplicacoes/testes_exemplos/5779779 (2).pdf][exame]]
** 1
*** a
y_hat = beta_00_hat + beta_01_hat*X
O beta_00_hat é o valor esperado da tensão arterial diastólica quando o valor esperado de beta_01_hat é zero.
O beta_01_hat é o valor esperado do aumento na tensão arterial por cada aumento unitário na idade.
*** b
#+begin_src R :session res_exame_01 :results output
  ...
  sst <- ssr+sse
  r_squared <- 1-(sse/sst)
  r_squared_adjusted <- 1-((sse/6)/(sst/7))
  r_squared
  r_squared_adjusted
...
#+end_example
#+begin_comment
Ao nível de significância de 5% há evidência estatística de que a variável explicativa é significativa, ou seja, que tem impacto na variável resposta.
#+end_comment

*** f
#+begin_src R :session res_exame_01 :results output
  predict(model, new=data.frame(X=10), interval="prediction")
#+end_src

#+RESULTS:
:   fit      lwr      upr
: 1  72 54.59039 89.40961
...
#+RESULTS:
:                    5 %      95 %
: (Intercept) 33.3410871 63.992246
: X            0.7526205  3.914046

* 11_12_2024
** function
#+begin_src R :session 11_12_2024__01 :results output
  loocv_mse <- function(data, formula) {
    n <- nrow(data)
    mse_values <- numeric(n)
    for(i in 1:n) {
...
#+RESULTS:
** abordagem validation approach
#+begin_src R :session 11_12_2024__01 :results output
  data(mtcars)
  dim(mtcars)
  set.seed(1)
  train <- sample(32,16)
  lm.fit <- lm(mpg~wt+am, data=mtcars, subset=train)
  mean((mtcars$mpg-predict(lm.fit, mtcars))[-train]^2)
  ## predict(lm.fit, mtcars)
  ## fitted(lm.fit)
  mean((mtcars$mpg-fitted(lm.fit))^2)
#+end_src
...
** [[file:~/Documents/CD/year_2/semester_01/modelos_lineares_aplicacoes/testes_exemplos/TesterecursoML202223.pdf][testerecursoML]]
*** a
**** primeiro passo
#+begin_src  R :session 11_12_2024__02 :results output
  data(iris)
  complete_model <- lm(iris$Sepal.Width~., data=iris)
  table(iris$Species) # espécie de referência
...
Call:
lm(formula = iris$Sepal.Width ~ Sepal.Length + Petal.Length +
    Petal.Width + Species, data = iris)

Coefficients:
      (Intercept)       Sepal.Length       Petal.Length        Petal.Width  Speciesversicolor   Speciesvirginica
           1.6572             0.3778            -0.1876             0.6257            -1.1603            -1.3983
#+end_example
#+begin_comment
O valor AIC mais baixo é aquele correspondente ao modelo com todas as variáveis.
Com o comando table(iris$Species) Verificamos que a referência é a setosa. Ou seja as variáveis dummy são Speciesversicolor se Speciesvirginica.
sepal.width_hat = 1.65 + 0.3778*Sepal.Length -0.187*Petal.Length + 0.6257*Petal.Width - 1.16*Speciesversicolor - 1.3983*Speciesvirginica
Beta_Sepal.Length_hat = 0.3778 é o aumento do valor esperado do Sepal.Width quando há o aumento de uma unidade de Sepal.Length mantendo constante as outras variáveis.
#+end_comment

*** a (02) Escrever o modelo ajustado da espécie Setosa.
#+begin_comment
sepal.width_hat = 1.65 + 0.3778*Sepal.Length -0.187*Petal.Length + 0.6257*Petal.Width
#+end_comment
...
#+RESULTS:
:                        2.5 %      97.5 %
: (Intercept)        1.1512680  2.16305976
: Sepal.Length       0.2481714  0.50737540
: Petal.Length      -0.3525966 -0.02253662
: Petal.Width        0.3818480  0.86957303
: Speciesversicolor -1.5423464 -0.77822421
: Speciesvirginica  -1.9460551 -0.85045466
...
Call:
lm(formula = iris$Sepal.Width ~ ., data = iris)
...
#+end_example
#+begin_comment
H0: beta_Sepal.Length = beta_Petal.Length=beta_Sepal.Width=beta_Species=0
H1: existe beta_j {beta_Petal.Length,beta_Sepal.Width,beta_Species} !=
ET = 50.14
P_value = 2.2*10^(-16)
Rejeitar H0 ...
#+end_comment
...
*** d
#+begin_src  R :session 11_12_2024__02 :results output
  model_no_species <- lm(iris$Sepal.Width~.-Species, data=iris)
  anova(model_no_species, complete_model)
#+end_src
...
Model 1: iris$Sepal.Width ~ (Sepal.Length + Petal.Length + Petal.Width +
    Species) - Species
Model 2: iris$Sepal.Width ~ Sepal.Length + Petal.Length + Petal.Width +
    Species
  Res.Df    RSS Df Sum of Sq      F    Pr(>F)
1    146 13.474
2    144 10.328  2    3.1464 21.936 4.831e-09 ***
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
#+end_example

** 2
#+begin_src  R :session 11_12_2024__03 :results output

  Y <- c(100,50,70,50,80,25,40,50,100,55)
  x <- c(6,5,5,5,6,4,5.5,6,8,6)
  A <- c(1,1,1,0,0,1,0,0,1,1)
  one <- c(rep(1,10))
  X <- matrix(c(one, x, A), ncol=3)
...
Residuals:
    Min      1Q  Median      3Q     Max
-17.516 -12.211  -6.289  12.765  27.484

Coefficients:
            Estimate Std. Error t value Pr(>|t|)
(Intercept)  -43.701     32.710  -1.336   0.2233
x             17.547      5.598   3.135   0.0165 *
A             10.936     11.441   0.956   0.3710
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

Residual standard error: 17.72 on 7 degrees of freedom
Multiple R-squared:  0.6082,	Adjusted R-squared:  0.4963
F-statistic: 5.433 on 2 and 7 DF,  p-value: 0.03765
#+end_example
** grupo 2 - 1
#+begin_src  R :session 11_12_2024__04 :results output
  a = 31/8
  b = (0.6679)/(1-(0.6679))
  a*b
